<script lang="ts">
    ...
    if (lPassword.length < 8) {
      return err = "Password must contain atleast 8 characters";
    }
		try {
    ...
    if (sPassword.length < 8 || sPassword != sCPassword) return;
		try {
			const resp = await axios.post(
				`${API_URL}/newuser`,
				{
...
<div class="flex w-full flex-col items-center gap-4">
	{#if err}
		<Alert.Root variant="destructive" class="w-full md:w-[36rem] bg-destructive/10">
			<Alert.Title>{err}</Alert.Title>
		</Alert.Root>
	{/if}
	<Tabs.Root value={data.form} class="w-full md:w-[36rem] bg-white">
		<Tabs.List class="w-full">
			<Tabs.Trigger value="login">Login</Tabs.Trigger>
			<Tabs.Trigger value="signup">Sign Up</Tabs.Trigger>
		</Tabs.List>
		<Tabs.Content value="login" class="px-8 py-10">
		...
		<Tabs.Content value="signup" class="px-8 py-10">
          ...
          required
				/>
				{#if sPassword.length > 0 && sPassword.length < 8}
					<p class="text-red-400">Password must contain atleast 8 characters</p>
				{/if}
          ...
          required
				/>
				{#if sCPassword.length > 0 && sPassword != sCPassword}
					<p class="text-red-400">Confirm password does not match the password</p>
				{/if}
				<div class="mt-2 flex items-center gap-1">
					<Checkbox
						id="sshowpw-{id}"
